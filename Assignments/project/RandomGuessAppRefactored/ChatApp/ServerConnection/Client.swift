...
class Client:NSObject, StreamDelegate {
    ...
    func stream(_ aStream: Stream, handle eventCode: Stream.Event) {
        ...
        case .errorOccurred:
            ...
            listenerDelegate?.clientReceivedError(errorString: str)
    ...
    private func readAndProcessData(from aStream:Stream) {
        ...
        for part in allData.split(separator:"\n") {
            ...
            if receivedData.dataType == .connectionYouConnected {
                ...
                listenerDelegate?.clientDidConnect(asPlayer: receivedData.player, andName: receivedData.username)
            }
            ...
            if receivedData.dataType == .connectionOrListenerError {
                listenerDelegate?.clientReceivedError(errorString: receivedData.userData)
            ...
            // what to do next
            listenerDelegate?.clientReceived(rawData: data)
        }
    }
